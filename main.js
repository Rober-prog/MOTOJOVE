  ...
  if (heroCanvas) {
    ...
    function createSpark() {
      return {
        ...
        y: heroCanvas.height + Math.random() * 40,
        vx: (Math.random() - .5) * .8,
        vy: -(1 + Math.random() * 2.2),
        r: .8 + Math.random() * 2,
        life: 0,
        maxLife: 80 + Math.random() * 120,
        hue: 30 + Math.random() * 15  // orange range
      };
    }
    ...
    function drawSparks() {
      ...
      sparks.forEach((s, i) => {
        s.x += s.vx + Math.sin(s.life * .04) * .3;
        s.y += s.vy;
        s.life++;
        const progress = s.life / s.maxLife;
        const alpha = progress < .1 ? progress / .1
                    : progress > .7 ? 1 - (progress - .7) / .3
                    : 1;
        if (s.life >= s.maxLife) {
